...
for t=1:G
    ...
    x(:,mu+1) = Recombination(x(:,r1),x(:,r2), x(:,r3), x(:,r4), x(:,r5));
    sigma(:,mu+1) = Recombination(sigma(:,r1),sigma(:,r2), sigma(:,r3), sigma(:,r4),sigma(:,r5));
...
disp([' mínimo global en: W=' num2str(x(1,ib)) ',' num2str(x(2,ib)) ',' num2str(x(3,ib)) ',' num2str(x(4,ib)) ',' num2str(x(5,ib)) ' f(W)=' num2str(f(x(:,ib)))])

%% Funciones
function y = Recombination (x1,x2, x3, x4 ,x5)
    y = 0.2*(x1+x2+x3+x4+x5);
end